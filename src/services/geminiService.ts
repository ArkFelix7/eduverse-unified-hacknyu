...
// ===============================
// Lecture Capture Services (Part 2)
// ===============================
    ...
    callbacks: {
      ...
      onmessage: (message: any) => {
        ...
        if (message.serverContent?.inputTranscription) {
          ...
          console.log('Processing transcription:', { text, isFinal });
          if (text && text.trim()) {
            onMessage(text, isFinal);
          }
        } else {
          console.log('Message does not contain inputTranscription:', message);
        }
      },
      onerror: (e: any) => {
...
// Analysis schema for lecture processing
...
export const analyzeLecture = async (transcript: string, audioBlob?: Blob): Promise<LectureAnalysis> => {
  ...
  // If audio blob is provided, include it for multimodal analysis
